...
Object.assign(I18N.en,{
  ...
  directory_zh:"Chinese search directory",directory_en:"English search directory",sitemap_label:"Sitemap",resource_open:"Open resource",team_page:"FIRST team page",cad_preview:"CAD preview",cad_preview_alt:"CAD preview for Team {team}",views_suffix:" views"
});
...
Object.assign(I18N['zh-CN'],{cad_preview_alt:"{team} 队 CAD 预览"});
Object.assign(I18N['zh-TW'],{cad_preview_alt:"{team} 隊 CAD 預覽"});
...
Object.assign(I18N.en,{ftc_built_by:"FIRSTHub, built by FRC Team 5449 for the FTC community."});
Object.assign(I18N['zh-CN'],{ftc_built_by:"由 FRC Team 5449 为 FTC 社区建设的 FIRSTHub。"});
Object.assign(I18N['zh-TW'],{ftc_built_by:"由 FRC Team 5449 為 FTC 社群建置的 FIRSTHub。"});
...
{cat:"data",t:"Citrus Circuits 侦察白皮书 (1678)",u:"https://www.citruscircuits.org/_files/ugd/146bfa_f8ccc9ba56bd49f58f56b9a6362e2627.pdf",d:"1678 公开的侦察系统白皮书，介绍其数据采集、分析与比赛策略流程。"},
...
"https://www.citruscircuits.org/_files/ugd/146bfa_f8ccc9ba56bd49f58f56b9a6362e2627.pdf":["Citrus Circuits Scouting Whitepaper (FRC 1678)","FRC 1678's public scouting-system whitepaper covering its data collection, analysis, and match-strategy workflow."],
...
function ftcAutoItems(){
  ...
  if(ftcCategory==='portfolios')return FTC_AUTO_DATA.portfolios.map(x=>{
    // Some older PortfolioLab exports accidentally embedded the following JSON fields in `award`.
    // Keep the card compact and expose only the short award/rating/score summary.
    const clean=(value,fallback='')=>{
      if(typeof value!=='string')return value||fallback;
      const text=value.replace(/\\\\"/g,'"').trim();
      const cut=text.search(/[,\\"](?:pdf|pdfStoragePath|source|eventName|summary)\\"?\\s*:/i);
      const short=(cut>0?text.slice(0,cut):text).replace(/[\\"{},]+$/,'').trim();
      return short.length<=80?short:(fallback||short.slice(0,77)+'…');
    };
    const award=clean(x.award,t('ftc_all_levels'));
    const rating=clean(x.rating);
    const score=clean(x.score);
    return {season:x.season,type:x.sourceType==='official'?'official':'community',detail:x.level||award,number:x.teamNumber,meta:(x.seasonLabel||'Season')+(x.level?' · '+x.level:''),title:'#'+x.teamNumber+' '+(x.teamName||x.title||'FTC Team'),desc:[award,rating,score].filter(Boolean).join(' · ')||t('ftc_public_portfolio'),source:String(x.source||'').includes('portfoliolab')?'FTC PortfolioLab':String(x.source||'').includes('open-vault-ftc')?'OpenVault':ftcLabel('Team-published'),url:x.pdf||x.source};
  });
  if(ftcCategory==='open')return FTC_AUTO_DATA.openTeams.map(x=>({season:x.season,type:'team',detail:(x.tags||['build-thread'])[0],tags:x.tags||['build-thread'],links:x.links||[],number:x.teamNumber||999999,activity:x.activity||x.views||x.posts||0,sourcePlatform:x.sourcePlatform||'chief-delphi',meta:(x.teamNumber?t('ts_n')+' '+x.teamNumber+' · ':'')+(x.views?Number(x.views).toLocaleString()+' '+t('ftc_views'):(x.posts||0).toLocaleString()+' '+t('ftc_updates')),title:(x.teamName?x.teamName+' — ':'')+x.title,desc:x.sourcePlatform==='youtube'?t('ftc_public_video'):t('ftc_public_team_resource'),source:x.sourcePlatform==='youtube'?(x.publisher?'YouTube · '+x.publisher:'YouTube'):x.sourcePlatform==='github'?'GitHub':'Chief Delphi',url:x.source}));
