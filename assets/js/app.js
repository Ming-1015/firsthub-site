...
function ftcAutoItems(){
  ...
  if(ftcCategory==='portfolios')return FTC_AUTO_DATA.portfolios.map(x=>({season:x.season,type:x.sourceType==='official'?'official':'community',detail:x.level||x.award||t('ftc_all_levels'),number:x.teamNumber,meta:(x.seasonLabel||'Season')+(x.level?' · '+x.level:''),title:'#'+x.teamNumber+' '+(x.teamName||x.title||'FTC Team'),desc:[x.award,x.rating,x.score].filter(Boolean).join(' · ')||t('ftc_public_portfolio'),source:x.source.includes('portfoliolab')?'FTC PortfolioLab':x.source.includes('open-vault-ftc')?'OpenVault':ftcLabel('Team-published'),url:x.pdf||x.source}));
